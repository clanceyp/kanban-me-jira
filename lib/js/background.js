    ...
    var DEFAULT_VALUES = window.DEFAULT_VALUES,
        ...
        kmj = {
            ...
            updateBrowserActionStatus:function(status, undefined) {
                ...
                return window.chrome.browserAction.setBadgeText({text: status.toString() });
            },
            /**
            ...
            /**
             * Method gets jira profile json for a user id
             * @id  getJiraProfileJSON
             * @memberOf kmj
             * @param {string} user jira user id
             * @return void
             *
             */
            getJiraProfileJSON:function(user, cb){
                function getUserJson(user, cb) {
                    var url = kmj.getLocalStore("JiraURL") + "/" + kmj.getLocalStore("JIRAProfileJSONurl") + user;
                    kmj[user] = $.ajax({
                        type: 'GET',
                        url: url,
                        dataType: "json",
                        ...
                        error: function (xhr, type) {
                            kmj[user] = null;
                            console.log("error in getJiraProfileJSON ", type);
                        }
                    });
                }
                new getUserJson(user, cb);
            }
        };
