(function($, _, can, chrome, d3, less, window){
    ...
    var backgroundPage = chrome.extension.getBackgroundPage(),
        kanban = {
            columnWidth:0,
            columnMon:{},
            userAvatarURL:{"-1":backgroundPage.kmj.getLocalStore("AvatarJIRADefault", 0, 0)},
            dirty : backgroundPage.kmj.getLocalStore("TicketDirty", 0, 0),
            avatarUseJIRAProfile: backgroundPage.kmj.getLocalStore("AvatarUseJIRAProfile", null, function(val){return val === 'true'}),
            TicketThumbnailBase: backgroundPage.kmj.getLocalStore("TicketThumbnailBase"),
            avatarIsGravitar: backgroundPage.kmj.getLocalStore("TicketThumbnailBase", "", function(val){ return val.indexOf(".gravatar.com")>0}),
            JIRAProfileJSONurl: backgroundPage.kmj.getLocalStore("JIRAProfileJSONurl",""),
            ...
            kanbanTemplate:null,
            baseZIndex: 1000,
            checktimer:(backgroundPage.kmj.getLocalStore('RefreshTime', 0, function(val){return parseInt(val, 10) || 0; }) * 1000),
            columnList:[],
            d3Selection:null,
            init:function(){
                ...
                if (backgroundPage.kmj.kanbanItems.length === 0){
                    kanban.timer = setTimeout(kanban.init, 5000);
                } else {
            ...
            getJiraProfileJSON:function(user){
                if (kanban.avatarUseJIRAProfile){
                    if (kanban.userAvatarURL[user]){
                        kanban.setAvatarSrc(kanban.userAvatarURL[user], user)
                    } else {
                        backgroundPage.kmj.getJiraProfileJSON(user, kanban.setAvatarURLFromProfile);
                    }
                }
            },
            setAvatarURLFromProfile:function(json){
                var name = json.name;
                kanban.userAvatarURL[name] = json.avatarUrls["48x48"];
                kanban.setAvatarSrc(kanban.userAvatarURL[name], name);
            },
            setAvatarSrc:function(src, name){
                can.$("img[data-assignee='"+ name +"']").each(function(i,el){
                    $(el).attr("src", src);
                });
            },
            ...
            postRender:function(){
                ...
                $("article.ticket img").each(function(i, el){
                    var assinee = getAssigneeName( $(el).attr("data-assignee") ),
                        src,
                        ticketThumbnailBase = kanban.TicketThumbnailBase;
                    if (kanban.ticketThumbnailForceSize){
                        $(el).addClass('fixedsize');
                    }
                    $(el).closest("article.ticket").attr("data-assinee", assinee);

                    if (assinee === "-1") {
                        if (!!kanban.ticketThumbnailBaseFallback) {
                            src = kanban.ticketThumbnailBaseFallback;
                        }
                    } else if (kanban.avatarUseJIRAProfile){
                        kanban.getJiraProfileJSON(assinee);
                        return;
                    } else if (kanban.avatarIsGravitar){
                        if (ticketThumbnailBase.indexOf('$user.name') > -1) {
                            // jira might not be using email address as assinee@name
                            // thumbnail location might be
                            // https://www.gravatar.com/avatar/$user.name@example.com
                            //
                            var bits = ticketThumbnailBase.split("$user.name");
                            var email = assinee;
                            if (bits.length > 1) {
                                email = email + bits.pop();
                            }
                            src = bits.shift() + md5(email);
                        }
                    } else {
                        if (ticketThumbnailBase.indexOf('$user.name') > -1) {
                            ticketThumbnailBase = ticketThumbnailBase.replace("$user.name", assinee);
                        }
                        src = ticketThumbnailBase;
                    }
    ...
    $(window).on('load',function(){
        ...
        // reload page every 3 hours; (or it will crash eventually)
        setTimeout(function(){
            location.reload()
        }, 1000 * 60 * 180);
    });
