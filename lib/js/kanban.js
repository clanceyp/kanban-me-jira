        ...
        kanban = {
            ...
            d3display:function(container, data){
                ...
                element.enter()
                    ...
                    .style('background-image', 'url(/lib/i/coffee001.png), url(/lib/i/coffee002.png), url(/lib/i/lined_paper_transparent.png)' )
                    .style('background-position', kanban.getRandomBGPosition )
                    .style('background-repeat', " no-repeat, no-repeat, repeat" );
            ...
            postRender:function(){
                $("article.ticket img").each(function(i, el){
                    ...
                    // console.log("src: " +src);
                    $(el).on('error',function(){$(el).remove();});
                    $(el).attr('src', src);
                });
                less.modifyVars({
            ...
            getRandomBGPosition:function(){
                var dirty = backgroundPage.kmj.getLocalStore("TicketDirty", 0, 0),
                    r = function(d){
                        return kanban.rand(-d, d) / dirty;
                    };
                if (dirty === 0){
                    return '-999px -999px, -999px -999px';
                }
                return  r(600) + 'px ' +r(600) +'px, '+r(1000)+'px '+r(1000)+'px';
            },
            getTop:function(d){
