	...
	window.popup = {
		...
		render:function(){
			...
			if (popup.items.length) {
				...
				var html = (url) ? 'Please <a href="'+ url +'">login</a> and/or check the <a href="options.html">options</a> page.' : 'Please check the <a href="options.html">configuration page</a>.';
		...
		monitor:function(){
			...
			if (popup.items.length === 0){
				$("p.message").empty().html( "Either your not logged in, or your filter ("+ backgroundPage.kmj.getLocalStore("PopupFilterID") +") is returning 0 issues! <br/>Click on the title above to check." );
			}
		}
	}
